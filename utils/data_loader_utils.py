...
def subsample_flow_vectors(u, v, k=5):
    """Return every kth vector from the flow field."""
    ys = np.arange(0, u.shape[0], k)
    xs = np.arange(0, u.shape[1], k)
    xv, yv = np.meshgrid(xs, ys)
    return xv, yv, u[::k, ::k], v[::k, ::k]
...
def create_input_output_animation_gif(dataset, dataloader, flow_x, flow_y, start_idx=0, num_frames=12, save_path=None, interval=800, input_channels_to_show=None,):
    """
    ...
    for i, channel_idx in enumerate(input_channel_indices):
        ...
        qx, qy, qu, qv = subsample_flow_vectors(
            flow_x,
            flow_y,
            k=5
        )
